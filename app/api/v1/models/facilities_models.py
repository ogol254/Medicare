...
class FacilityModel(BaseModel):
    ...
    def get_all(self):
        ...
        curr.execute("""SELECT facility_id, name, location, created_by, 
                        contact FROM facilities;""")
        data = curr.fetchall()
        resp = []
        curr.close()

        for i, items in enumerate(data):
            facility_id, name, location, created_by, contact = items
    ...
    def save_facility(self):
        ...
        query = """INSERT INTO facilities (name, location, created_by, contact) \
            VALUES ( %(name)s, %(location)s, %(created_by)s, %(contact)s) RETURNING facility_id;
            """
    ...
    def get_single_facility(self, facility_id):
        ...
        query = """SELECT facility_id, name, location, created_by, 
                        contact FROM facilities WHERE facility_id=%s"""
        curr.execute(query, [facility_id])
        data = curr.fetchone()
        resp = []
        curr.close()

        facility_id, name, location, created_by, contact = data
